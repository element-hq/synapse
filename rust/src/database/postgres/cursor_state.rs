...
impl<S: CursorRowStream> CursorQueryState<S> {
    ...
    pub fn fetch_one<'py>(
        ...
        match stream.as_mut().block_on_next(py, handle) {
    ...
    pub fn fetch_next_batch<'py>(
        ...
        match pull_ready_batch(stream, py, handle, capacity) {
    ...
    pub fn fetch_all<'py>(
        ...
        let rows = match stream.as_mut().try_collect::<Vec<_>>().block_on(py, handle) {
            Ok(rows) => rows,
            Err(err) => {
                *self = Self::Idle;
    ...
    pub fn rowcount<'py>(
        ...
        {
            if let Err(err) = drain_stream(stream.as_mut()).block_on(py, handle) {
                *self = Self::Idle;
                return Err(S::stream_err(&err));
            }
            let rowcount = rows_affected(stream);
...
fn pull_ready_batch<'py, S: CursorRowStream>(
    ...
    let first = match stream.as_mut().block_on_next(py, handle) {
...
mod tests {
    ...
    /// A process-wide runtime handle for the tests. [`FakeStream`] is always
    /// `Poll::Ready`, so nothing here ever actually blocks on the runtime; the
    /// helpers just need *a* handle to satisfy their signature (in production it
    /// is the shared runtime's, carried by the connection).
    ...
    fn fetch_one_yields_rows_then_reports_and_closes() {
        ...
        Python::attach(|py| {
            ...
            assert_tuple(&state.fetch_one(py, handle()).unwrap().unwrap(), &[1]);
            assert_tuple(&state.fetch_one(py, handle()).unwrap().unwrap(), &[2]);
            // The end of the stream is reported as `None`, closing the cursor.
            assert!(state.fetch_one(py, handle()).unwrap().is_none());
    ...
    fn fetch_all_drains_and_closes() {
        ...
        Python::attach(|py| {
            ...
            let rows = state.fetch_all(py, handle()).unwrap();
    ...
    fn fetch_all_on_empty_stream_returns_empty_and_closes() {
        ...
        Python::attach(|py| {
            ...
            assert!(state.fetch_all(py, handle()).unwrap().is_empty());
            assert!(matches!(state, CursorQueryState::Closed { .. }));
        });
    }
    ...
    fn fetch_one_then_fetch_all_continues_partially_consumed_stream() {
        ...
        Python::attach(|py| {
            ...
            let rest = state.fetch_all(py, handle()).unwrap();
    ...
    fn fetch_all_error_resets_to_idle() {
        ...
        Python::attach(|py| {
            ...
            let err = state.fetch_all(py, handle()).unwrap_err();
    ...
    fn rowcount_after_drain_error_resets_to_idle() {
        ...
        Python::attach(|py| {
            ...
            let err = state.rowcount(py, handle()).unwrap_err();
    ...
    fn fetch_next_batch_returns_ready_rows_then_defers_the_empty_report() {
        ...
        Python::attach(|py| {
            ...
            let mut state = active_with(vec![vec![1], vec![2], vec![3]], Some(0));
            let batch = state.fetch_next_batch(py, handle(), 100).unwrap();
            ...
            // moves to `Closed`.
            assert!(state
                .fetch_next_batch(py, handle(), 100)
                .unwrap()
                .is_empty());
            assert!(matches!(state, CursorQueryState::Closed { .. }));
    ...
    fn fetch_next_batch_capacity_is_a_hint_not_a_limit() {
        ...
        Python::attach(|py| {
            ...
            let mut state = active_with(vec![vec![1], vec![2], vec![3]], Some(0));
            let batch = state.fetch_next_batch(py, handle(), 1).unwrap();
            assert_eq!(batch.len(), 3);
        });
    }
    ...
    fn fetch_next_batch_error_resets_to_idle() {
        ...
        Python::attach(|py| {
            ...
            let err = state.fetch_next_batch(py, handle(), 100).unwrap_err();
