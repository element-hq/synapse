...
/// Fill unset [`TlsParams`] fields from libpq's `PGSSL*` environment
/// variables, as libpq does for keywords the connection string omits.
///
/// Called at pool/connection construction (see `ConnectionManager::from_dsn`,
/// next to the other `PG*` env handling in `fixup_config_defaults`) rather
/// than inside [`build`], which stays a pure function of its params.
///
/// Deliberately more conservative than libpq where a stray variable could
/// break a working setup:
///  - a set-but-empty variable is treated as unset, as libpq does;
///  - `PGSSLCERT`/`PGSSLKEY` are imported only as a complete pair, and only
///    when neither is configured explicitly. libpq resolves a lone sslcert's
///    key lazily (`~/.postgresql/postgresql.key`) and only fails if the
///    server actually requests a client certificate; we build the TLS config
///    eagerly, so a leftover `PGSSLCERT` from some other tool must not abort
///    every connection.
pub(crate) fn with_env_defaults(params: &TlsParams) -> TlsParams {
    fn env_nonempty(var: &str) -> Option<String> {
        std::env::var(var).ok().filter(|value| !value.is_empty())
    }
    fn fallback(value: &Option<String>, var: &str) -> Option<String> {
        value.clone().or_else(|| env_nonempty(var))
    }

    let (sslcert, sslkey) = match (&params.sslcert, &params.sslkey) {
        (None, None) => match (env_nonempty("PGSSLCERT"), env_nonempty("PGSSLKEY")) {
    ...
    TlsParams {
        sslmode: fallback(&params.sslmode, "PGSSLMODE"),
        sslrootcert: fallback(&params.sslrootcert, "PGSSLROOTCERT"),
        sslcert,
        sslkey,
        sslpassword: fallback(&params.sslpassword, "PGSSLPASSWORD"),
    }
}

/// Resolve [`TlsParams`] into the `tokio-postgres` [`SslMode`] to set on the
/// connection config and the [`TlsConnector`] to connect with. A pure
/// function of its params: the `PGSSL*` env fallbacks are applied by the
/// caller (see [`with_env_defaults`]).
...
/// Load a PEM private-key file. Encrypted keys are not supported by
/// `rustls-pemfile`; if no usable key is found and an `sslpassword` was
/// supplied, say so in the error. A password alongside an *unencrypted* key
/// is simply ignored, as libpq ignores it — a stray `PGSSLPASSWORD` in the
/// environment must not break working mutual TLS.
fn load_key(path: &str, sslpassword: Option<&str>) -> anyhow::Result<PrivateKeyDer<'static>> {
    ...
    {
        Some(key) => Ok(key),
        None if sslpassword.is_some() => Err(anyhow!(
            "no usable private key found in {path}: encrypted client keys \
             (sslpassword) are not supported by the Rust driver"
        )),
...
impl ServerCertVerifier for NoHostnameServerCert {
    fn verify_server_cert(
        ...
        tolerate_name_mismatch(self.inner.verify_server_cert(
            end_entity,
            intermediates,
            server_name,
            ocsp_response,
...
/// Map a hostname-mismatch verification error to success, for `verify-ca`.
/// rustls reports mismatches as `NotValidForNameContext` (`NotValidForName` is
/// its context-free legacy spelling; match both in case a future verifier
/// reverts to it). Everything else — expiry, unknown CA, … — passes through.
fn tolerate_name_mismatch(
...
mod tests {
    ...
    #[test]
    fn verify_ca_tolerates_both_hostname_mismatch_spellings() {
        // The context-carrying variant rustls's webpki verifier actually
        // returns for a hostname mismatch...
        let ctx = CertificateError::NotValidForNameContext {
            expected: ServerName::try_from("db.example.com").unwrap(),
            presented: vec!["DnsName(\"other.example.com\")".to_owned()],
        };
        assert!(tolerate_name_mismatch(Err(RustlsError::InvalidCertificate(ctx))).is_ok());

        // ...and its context-free legacy spelling.
        assert!(tolerate_name_mismatch(Err(RustlsError::InvalidCertificate(
            CertificateError::NotValidForName
        )))
        .is_ok());

        // Any other certificate failure still propagates.
        assert!(tolerate_name_mismatch(Err(RustlsError::InvalidCertificate(
