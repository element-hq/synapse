...
impl Connection {
    ...
    ///
    /// Blocking DB calls on this connection (and on cursors opened from it)
    /// drive their futures on the runtime the *calling thread* has entered (see
    /// [`super::helpers::BlockingPostgres`]), so the connection itself carries
    /// no runtime handle.
