...
impl Connection {
    ...
    fn cursor<'py>(&self, py: Python<'py>) -> PyResult<(Bound<'py, Cursor>, CursorGuard)> {
        ...
        // If building the Python handle fails here — after `BEGIN` but before
        // the `CursorGuard` exists — both `Cursor` clones drop, which drops the
        // client and closes the socket (the server rolls back the open
        // transaction). The connection is left closed, matching the
        // drop-on-error policy in the module docs.
...
impl CursorInner {
    ...
    fn execute<'py>(
        ...
        // The column names back the (future) PEP-249 `Cursor.description`; pull
        // them out of the prepared statement here so `cursor_state` stays
        // decoupled from `tokio_postgres::Column`.
        ...
        self.query_state.on_query_start(row_stream, description);
