...
pub(crate) fn http_request_from_twisted(request: &PyAny) -> PyResult<Request<Bytes>> {
...
///
/// Returns an error if the Python object doens't properly implement ``IRequest``
pub(crate) fn http_response_to_twisted<B>(request: &PyAny, response: Response<B>) -> PyResult<()>
where
    B: Buf,
{
