        ...
        main_sync_body = {
            "lists": {
                ...
                "foo-list": {
                    "ranges": [[0, 1]],
                    "required_state": [],
                    # We set this to `1` because we're testing `receipts` which interact
                    # with the `timeline`. With receipts, when a room hasn't been sent
                    # down the connection before or it appears as `initial: true`, we
                    # only include receipts for events in the timeline to avoid bloating
                    # and blowing up the sync response as the number of users in the
                    # room increases. (this behavior is part of the spec)
                    "timeline_limit": 1,
                },
