...
class RoomSummaryTestCase(unittest.HomeserverTestCase):
    ...
    # Since the majority of the fields in a room summary comes from the stats tables,
    # and those are only generated by the worker/main_process if designated to run
    # background tasks, disable the background tasks.
    @override_config({"run_background_tasks_on": "non-existent-worker"})
    def test_join_rules(self) -> None:
        """join_rules should be populated immediately"""
        result = self.get_success(self.handler.get_room_summary(self.user, self.room))
        # The join_rules are specifically set to "invite"
        self.assertEqual(result.get("join_rule"), JoinRules.INVITE, result)
        ...
        result = self.get_success(self.handler.get_room_summary(self.user, self.room))
        self.assertEqual(result.get("join_rule"), JoinRules.PUBLIC, result)
