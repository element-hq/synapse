...
def connect(reactor: ISynapseReactor, dsn: str) -> Connection:
    """Open a new Postgres connection from a libpq-style DSN.

    ``reactor`` is the Twisted reactor the extension's shared tokio runtime is
    stored on; the runtime is started on demand if the reactor hasn't run yet
    (so this works during schema setup and in tests). Blocks until the
