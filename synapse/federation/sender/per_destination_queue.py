    ...
    async def _transaction_transmission_loop(self) -> None:
        ...
        try:
            ...
            while self._transmission_loop_enabled:
                self._new_data_to_send = False
                # Clear `transaction` (as it's used in the exception handler
                # for error reporting)
                transaction = None
    ...
    async def _prepare_transaction(self) -> _PreparedTransaction | None:
        ...
        Side effects:
            - Dequeues pending EDUs
                ...
                - `_pending_edus_keyed`
            - Advances our devices stream positions (only if there is nothing to send
              in that range, so we don't lose any updates)
